...
/// <summary>原生模型窗口的宿主服务，复用既有业务命令并严格隔离来源权限。</summary>
public sealed class ModelService : IDisposable
{
	...
	/// <summary>模型或运行时状态变化通知，订阅方负责切回 UI 线程。</summary>
	public event Action? StateChanged;

	/// <summary>创建绑定到模型窗口的宿主服务。</summary>
	public ModelService(AppServices services, Window owner)
	{
		_services = services ?? throw new ArgumentNullException(nameof(services));
		_context = new ModelContext(owner ?? throw new ArgumentNullException(nameof(owner)));
		_router = new BridgeCommandRouter(services);
		if (services.Runtime is { } runtime) runtime.StateChanged += RaiseStateChanged;
	}

	/// <summary>原生模型窗口允许执行的完整命令集合。</summary>
	public static IReadOnlySet<string> Commands => AllowedCommands;

	/// <summary>检查命令是否属于原生模型权限范围。</summary>
	...
	{
		if (source is INativeModelSource && !IsCommandAllowed(command))
			throw new InvalidOperationException($"原生模型窗口不允许执行命令: {command}");
	}

	/// <summary>后台执行模型命令，返回与既有桥接一致的 JSON。</summary>
		...
		CancellationToken cancellationToken = default)
	{
		...
		try
		{
			...
			using CancellationTokenSource linked = CancellationTokenSource.CreateLinkedTokenSource(
				...
				IsBackgroundCommand(command) ? backgroundToken : CancellationToken.None);
			object? result = await Task.Run(
				() => _router.InvokeAsync(_context, command, commandArgs, linked.Token),
	...
	/// <summary>取消当前查询，保留导入和普通保存，并允许隐藏后重新查询。</summary>
