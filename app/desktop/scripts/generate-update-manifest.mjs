...
for (const KEY of ["version", "rid", "archive-path", "publish-dir"]) {
	if (!ARGS.get(KEY)) throw new Error(`缺少必要参数: --${KEY}`)
}
const VERSION = validateProductVersion(ARGS.get("version"))
...
const REPOSITORY = ARGS.get("repo") ?? "MF-Dust/Nori-Desktop-Pet"
if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(REPOSITORY)) throw new Error("GitHub 仓库名称无效")
const PUBLISH = path.resolve(ARGS.get("publish-dir"))
...
if (DEPLOYMENT.schema_version !== 1 || DEPLOYMENT.product_version !== VERSION || DEPLOYMENT.numeric_version !== NUMERIC
	...
	|| (ARGS.has("revision") && validateRevision(ARGS.get("revision")) !== REVISION)) {
...
if (typeof ENTRYPOINT !== "string" || ENTRYPOINT.includes("\\") || ENTRYPOINT.includes(":")
	...
	// eslint-disable-next-line -- Codacy误报：ENTRYPOINT已通过路径段校验
	|| !fs.statSync(path.join(PUBLISH, SLOT, ENTRYPOINT)).isFile()) throw new Error("发布槽入口无效") // nosemgrep
const ARCHIVE = path.resolve(ARGS.get("archive-path"))
...
const OUTPUT = path.resolve(ARGS.get("output-dir") ?? "bin/release")
