...
<template>
	<div class="w-full h-full flex flex-col gap-4 px-0.5 py-0.5 scroll-area">
		...
		<section
			v-if="!modelInstalled"
			class="shrink-0 flex items-center justify-between gap-3 rounded-md border border-warning/35 bg-warning/8 px-4 py-3"
			role="alert"
		>
		...
		<section
			v-if="SAFE_MODE"
			class="shrink-0 flex min-w-0 items-center gap-2.5 rounded-md border border-warning/35 bg-warning/8 px-4 py-3"
			role="status"
		>
			<Icon name="alert" :size="17" class="shrink-0 text-warning"/>
			<div class="flex min-w-0 flex-col gap-0.5">
		...
		<!-- 角色舞台 -->
		<section
			class="shrink-0 relative overflow-hidden flex flex-wrap items-center justify-between gap-4 p-5 rounded-lg
				bg-bg-card border border-line-strong
				shadow-elev-1"
		>

			<div class="relative flex items-center gap-5 min-w-0">
				<!-- 头像与在线呼吸环 -->
				<div class="relative w-[5.8rem] h-[5.8rem] shrink-0 rounded-full flex items-center justify-center bg-bg-deep/90 border-2 border-nori-teal-bright/35 overflow-hidden">
					<img :src="currentModel.thumb" :alt="currentModel.name" class="w-full h-full object-cover object-top transition-transform duration-200 hover:scale-110"/>
					<span
						...
						:style="props.petVisible ? {'--pulse-tint': 'var(--success)'} : undefined"
					/>
				</div>

				<div class="flex flex-col gap-1.5 min-w-0">
					<div class="flex items-center gap-2.5 flex-wrap">
						<h2 class="title-lg tracking-[0.02rem] text-text-primary">{{ currentModel.name }}</h2>
			...
			<div class="grid gap-2.5 grid-cols-1 md:grid-cols-3">
				<button
					...
					class="group relative overflow-hidden flex flex-col gap-2 p-4 text-left
						...
						hover:(border-line-strong bg-bg-card-hover)"
					@click="emit('navigate', card.key)"
				>

					<div class="flex items-center gap-2.5 min-w-0">
						<span class="w-8 h-8 shrink-0 rounded-sm flex items-center justify-center border border-line-subtle text-nori-teal-bright bg-nori-teal-bright/10 transition-all duration-200 group-hover:(border-nori-teal-bright/40)">
		...
		<!-- 生态社区 -->
		<section class="shrink-0 flex flex-col gap-2 pt-2 border-t border-line-subtle">
